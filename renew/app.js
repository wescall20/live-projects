...
document.getElementById("submit-request").addEventListener("click",async()=>{
  ...
  if(config.mode==="connected"&&config.endpoints.leadSubmission){
    ...
    if(!response.ok){alert("Renew could not receive the request yet. Please try again or contact Renew directly.");return;}
    const result=await response.json();state.leadId=result.leadId;
    if(state.photos.length&&config.endpoints.photoUpload){
      ...
      if(!photoResponse.ok){alert("Renew received the project request, but the photos did not upload. Mark can ask you for them during follow-up.");}
    }
    document.getElementById("confirmation-copy").textContent="Renew received your request. Mark will personally confirm the walkthrough.";
